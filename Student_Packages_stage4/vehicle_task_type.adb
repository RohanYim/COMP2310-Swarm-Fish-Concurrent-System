         ...
         Outer_task_loop : loop
            ...
            begin
               ...
               if Globes'Size > 0 then
                  ...
                  -- And myslef to survive_list
                  if Integer'Value (Message_from_car.survive_list.Length'Image) < Swarm_Size.Target_No_of_Elements then
                     -- if survive_list is not full, add me to the list
                     if Message_from_car.survive_list.Find_Index (Vehicle_No) >= 0 then
                        null;
                     else
                        Message_from_car.survive_list.Append (Vehicle_No);
                     end if;
                  end if;
                  ...
                  -- if I need to charge, compare if I am the most urgent one
                     ...
                     while Messages_Waiting loop
                        ...
                        if Ada.Real_Time.">="(Left  => Message_for_charge.Get_Time, Right => Lastest_Time) then
                           ...
                           Message_from_car.V_charge := Current_Charge;
                           -- update survive_list
                           Protected_Element.update_survive_list (Message_from_car, Message_for_charge, Vehicle_No);
                        end if;
                     end loop;
                     Send (Message => Message_from_car);
                     ...
                     -- no, go charging
                        ...
                        then abort
                           ...
                           -- check if I am in the survive_list, if so go charging, if not stay and die
                           if Integer'Value (Message_from_car.survive_list.Length'Image) < Swarm_Size.Target_No_of_Elements then
                  ...
                  if Current_Charge < 0.5 then
                     ...
                     while Messages_Waiting loop
                        ...
                        if Ada.Real_Time.">="(Left  => Get_Message_from_Car.Get_Time, Right => Lastest_Time) then
                           ...
                           Message_from_car.Get_Time := Lastest_Time;
                           Message_from_car.V_charge := Current_Charge;
                           Message_from_car.survive_list := Get_Message_from_Car.survive_list;
                        end if;
                     end loop;
                  ...
                  -- if I got enough charge
                     ...
                     while Messages_Waiting loop
                        ...
                        if Ada.Real_Time.">="(Left  => Get_Message_from_Car.Get_Time, Right => Lastest_Time) then
                           ...
                           Message_from_car.Get_Time := Lastest_Time;
                           Message_from_car.V_charge := Current_Charge;
                           Protected_Element.update_survive_list (Message_from_car, Get_Message_from_Car, Vehicle_No);
                        end if;
                     end loop;
                     -- send message to help others
                     Send (Message => Message_from_car);
